...
pop_data <- read_excel("Pop_Data/nst-est2019-01.xlsx")
names(pop_data)[1] <- "State"
names(pop_data)[2] <- "Population"


dataReg1 <- read_excel("Registrations_Data/Registrations-City-State-Country-Count-2015Q4-2018Q3.xlsx")
names(dataReg1)[1] = "Year"
names(dataReg1)[4] = "State"
names(dataReg1)[7] = "RegistrationCount"
dataReg1 <- dataReg1[dataReg1$Country == 'United States',]
dataReg1 <- dataReg1[-c(2,5,6)]
dataReg1 <- subset(dataReg1, dataReg1$State != "")
dataReg1$State <- state.name[match(dataReg1$State,state.abb)]
dataReg1$State[is.na(dataReg1$State)] <- "District of Columbia"


dataReg2 <- read_excel("Registrations_Data/Registrations-City-State-Country-Count-2019Q1.xlsx")
names(dataReg2)[2] = "State"
names(dataReg2)[5] = "RegistrationCount"
dataReg2 <- dataReg2[dataReg2$Country == 'United States',]
dataReg2 <- dataReg2[-c(3,4)]
dataReg2 <- subset(dataReg2, dataReg2$State != "")
dataReg2$State <- state.name[match(dataReg2$State,state.abb)]
dataReg2$Year <- '2019'
dataReg2$State[is.na(dataReg2$State)] <- "District of Columbia"


dataReg3 <- read_excel("Registrations_Data/Registrations-City-State-Country-Count-2019Q2.xlsx")
names(dataReg3)[2] = "State"
names(dataReg3)[5] = "RegistrationCount"
dataReg3 <- dataReg3[dataReg3$Country == 'United States',]
dataReg3 <- dataReg3[-c(3,4)]
dataReg3 <- subset(dataReg3, dataReg3$State != "")
dataReg3$State <- state.name[match(dataReg3$State,state.abb)]
dataReg3$Year <- '2019'
dataReg3$State[is.na(dataReg3$State)] <- "District of Columbia"


dataReg4 <- read_excel("Registrations_Data/Registrations-City-State-Country-Count-2019Q3.xlsx")
names(dataReg4)[2] = "State"
names(dataReg4)[5] = "RegistrationCount"
dataReg4 <- dataReg4[dataReg4$Country == 'United States',]
dataReg4 <- dataReg4[-c(3,4)]
dataReg4 <- subset(dataReg4, dataReg4$State != "")
dataReg4$State <- state.name[match(dataReg4$State,state.abb)]
dataReg4$Year <- '2019'
dataReg4$State[is.na(dataReg4$State)] <- "District of Columbia"


dataReg1$Year <- as.character(dataReg1$Year)
dataRegA <- bind_rows(dataReg1, dataReg2)
dataRegB <- bind_rows(dataRegA, dataReg3)
dataRegTOTAL <- bind_rows(dataRegB, dataReg4)
...
data1 <- read_excel("UAS_Sightings_Data/FY2019_Q3_UAS_Sightings.xlsx")
names(data1)[1] <- "Date"

data2 <- read_excel("UAS_Sightings_Data/FY2019_Q2_UAS_Sightings.xlsx")
names(data2)[1] <- "Date"

data3 <- read_excel("UAS_Sightings_Data/FY2019_Q1_UAS_Sightings.xlsx")
names(data3)[1] <- "Date"

data4 <- read_excel("UAS_Sightings_Data/FY2018_Q4_UAS_Sightings.xlsx")
names(data4)[1] <- "Date"

data5 <- read_excel("UAS_Sightings_Data/FY2018_Q3_UAS_Sightings.xlsx")
names(data5)[1] <- "Date"

data6 <- read_excel("UAS_Sightings_Data/FY2018_Q2_UAS_Sightings.xlsx")
names(data6)[1] <- "Date"

data7 <- read_excel("UAS_Sightings_Data/FY2018_Q1_UAS_Sightings.xlsx")
names(data7)[1] <- "Date"

data8 <- read_excel("UAS_Sightings_Data/FY2017_Q4_De-identification_Redaction_11192017.xlsx")
names(data8)[1] <- "Date"
names(data8)[2] <- "Summary"

data9 <- read_excel("UAS_Sightings_Data/UAS_sightings_report_Apr_Jun2017.xlsx")
names(data9)[1] <- "Date"
data9$Date <- as.Date(data9$Date, "%B %d, %Y")

data10 <- read_excel("UAS_Sightings_Data/UAS_sightings_report_Jan_Mar2017.xlsx")
names(data10)[1] <- "Date"
data10$Date <- as.Date(data10$Date, "%m/%d/%Y")

data11 <- read_excel("UAS_Sightings_Data/UAS_sightings_report_170331.xlsx")
names(data11)[1] <- "Date"
names(data11)[2] <- "City"
...
data12 <- read_excel("UAS_Sightings_Data/UAS_Sightings_report_1Jul-30Sep16.xlsx")
names(data12)[1] <- "Date"
names(data12)[2] <- "City"
names(data12)[3] <- "State"
names(data12)[4] <- "Summary"

data13 <- read_excel("UAS_Sightings_Data/UAS_Sightings_report_1Apr-30Jun16.xlsx")
names(data13)[1] <- "Date"
names(data13)[2] <- "City"
...
data14 <- read_excel("UAS_Sightings_Data/UAS_Sightings_report_1Feb-31Mar16.xlsx")
names(data14)[1] <- "Date"
names(data14)[2] <- "Summary"
names(data14)[3] <- "City"
names(data14)[4] <- "State"

data15 <- read_excel("UAS_Sightings_Data/UAS_Sightings_report_21Aug-31Jan.xlsx")
names(data15)[1] <- "Date"
names(data15)[2] <- "Summary"
names(data15)[3] <- "City"
names(data15)[4] <- "State"

data16 <- read_excel("UAS_Sightings_Data/UASEventsNov2014-Aug2015.xls")
names(data16)[1] <- "Date"
names(data16)[2] <- "City"
...
income_data <- read_excel("Income_Data/median_household_income_by_state_3yr_avg.xls")
